...
exports.utils.getPointsInGeometry = function(knex,fq_table_name,geometry,geom_column_name,return_column_names){
  return knex.select(return_column_names)
  .select(dboa.knex.raw('ST_Contains(' + geom_column_name + ',\''+geometry +'\')'))
  .where(dboa.knex.raw('ST_Contains(' + geom_column_name + ',\''+geometry +'\')'),true)
  .from(fq_table_name)
  .then(function(results){
    return results;
  });
