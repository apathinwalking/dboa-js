...
// args, column_names, verbose
exports.tidyAddr = function(tableObjs,args){
  var promises = _.map(_.range(0,tableObjs.length),function(i){
    var colObj = {
      knex: tableObjs[i].knex,
      fq_table_name:tableObjs[i].fq_table_name,
      column_name:args.column_names[i]
    }
    var arg = _.assign({},{
      verbose:false,
      tmp_table_name:"_tmp_table" + i,
      tmp_column_name:args.column_names[i] + "_tmp_idx" + i
    },_.omit(args,'column_names'));
    return soloColumnQuery.tidyAddr(colObj,arg);
  });
  return Promise.all(promises);
}
