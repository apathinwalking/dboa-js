...
exports.exists = function(tblObj){
  return tblObj.knex.select(tblObj.knex.raw('table_schema || \'.\' || table_name as fq_table_name'))
    .where(tblObj.knex.raw('table_schema || \'.\' || table_name'), tblObj.fq_table_name)
...
exports.dropIfExists = function(tblObj){
  return exports.exists(tblObj)
    .then(function(results){
      if(results === true){
        return tblObj.knex.schema.dropTable(tblObj.fq_table_name)
          .then(function(){
            return true;
          })
      }
...
exports.createPrimaryKeyIndexColumn = function(tblObj, args) {
    return exports.tableName(tblObj)
        .then(function(tableName) {
            ...
            return tblObj.knex.schema.table(tblObj.fq_table_name, function(table) {
                    table.bigIncrements(args.column_name).primary();
...
exports.createTableFromFlatPartition = function(tblObj, args){
    ...
    return query.then(function(results){
        var newTableObj = {fq_table_name:args.fq_table_name, knex:tblObj.knex}
...
// args [foreign_fq_table_name,primary_column_name,foreign_column_name,primary_data_column_name,foreign_data_column_name]
exports.createLinkColumn = function(tblObj, args){
  //TODO: fix that it doesn't work for non-int
  return tblObj.knex.schema.table(tblObj.fq_table_name,function(table){
    table.bigInteger(args.primary_column_name).references(args.foreign_column_name).inTable(args.foreign_fq_table_name);
  })
  .then(function(){
    var query = rawQuery.linkColumns({
      primary_fq_table_name:tblObj.fq_table_name,
      foreign_fq_table_name:args.foreign_fq_table_name,
      primary_column_name:args.primary_column_name,
      foreign_column_name:args.foreign_column_name,
      primary_data_column_name:args.primary_data_column_name,
      foreign_data_column_name:args.foreign_data_column_name
    });
    return tblObj.knex.raw(query);
  });
};
